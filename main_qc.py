...
from utils.config import get_config
from trainers import vificlip_no_qc as vificlip
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, config, mixup_fn):
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        label_id = label_id.reshape(-1).long()
        ...
        output_sqc = model(images)

        total_loss = criterion(output_sqc, label_id)
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            for i in range(n):
                ...
                output_qc = model(image_input)

                similarity = output_qc.view(b, -1).softmax(dim=-1)
                tot_similarity += similarity

            values_1, indices_1 = tot_similarity.topk(1, dim=-1)
            values_5, indices_5 = tot_similarity.topk(2, dim=-1)
            acc1, acc5 = 0, 0
            for i in range(b):
            ...
            if idx % config.PRINT_FREQ == 0:
                logger.info(
                    ...
                    f'Acc@2: {acc5_meter.avg:.3f}\t'
                )

    acc1_meter.sync()
    acc5_meter.sync()
    logger.info(f' * Acc@1 {acc1_meter.avg:.3f} Acc@2 {acc5_meter.avg:.3f}')
