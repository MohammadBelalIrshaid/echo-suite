...
class ViFiCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, logger):
        ...
        self.prompt_learner_no = VLPromptLearner(cfg, classnames, clip_model, logger, type = 'no')
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.text_encoder_no = copy.deepcopy(self.text_encoder)
    ...
    def forward(self, image):
        ...
        tokenized_prompts_no = self.prompt_learner_no.tokenized_prompts
        ...
        image_features1 = image_features.mean(dim=1, keepdim=False)  # image features are now ready
        image_features1_no = image_features_no.mean(dim=1, keepdim=False)  # image features are now ready
        ...
        yes_weights = torch.round(attention_weights * 10000) / 10000
        ...
        no_weights = torch.round(attention_weights_no * 10000) / 10000
        ...
        text_features_no = self.text_encoder_no(prompts, tokenized_prompts_no)

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        image_features_no = image_features_no / image_features_no.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_no = text_features_no / text_features_no.norm(dim=-1, keepdim=True)
        logits = logit_scale * image_features @ text_features.t()
        logits_no = logit_scale * image_features_no @ text_features_no.t()

        return logits, logits_no, text_features, text_features_no, image_features, image_features_no, yes_weights, no_weights
